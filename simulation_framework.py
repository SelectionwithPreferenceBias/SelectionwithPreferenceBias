...
def synth_simulation(n, p, k_inst, phi, dist, beta, type, iter, saveImg=False, showImg=True):
    ...
    plt.errorbar(gammas, gs_means, yerr=gs_stds, fmt='-', label='Unconstrained')
    plt.errorbar(gammas, group_means, yerr=group_stds, fmt='-', label='Group-wise constraints')
    plt.errorbar(gammas, inst_means, yerr=inst_stds, fmt='--', label='Unit-wise constraints')
    ...
    if saveImg:
        ...
        filename = os.path.join(subdirectory, f'{type}_{dist}_beta{beta}_phi{phi}_iter{iter}.png')
...
def jee_simulation(n, p, utils_norm, utils_bias, k_inst, pref, type, iter, limit, attr, saveImg=False, showImg=True):
    ...
    plt.errorbar(phis, gs_means, yerr=gs_stds, fmt='-', label='Unconstrained')
    plt.errorbar(phis, group_means, yerr=group_stds, fmt='-', color='orange', label='Group-wise constraints')
    plt.errorbar(phis, inst_means, yerr=inst_stds, fmt='--', color='green', label='Unit-wise constraints')
    ...
    if saveImg:
        ...
        filename = os.path.join(subdirectory, f'jee_{type}_{attr}_{iter}iter_{limit}limit.png')
        plt.savefig(filename)
