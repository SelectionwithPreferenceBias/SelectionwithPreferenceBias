...
def simulation_corrpref(n, p, k_inst, dist, beta, type, iter, saveImg=False, showImg=True):
    ...
    plt.errorbar(alphas, gs_means, yerr=gs_stds, fmt='-', label='Unconstrained')
    plt.errorbar(alphas, group_means, yerr=group_stds, fmt='-', label='Group-wise constraints')
    plt.errorbar(alphas, inst_means, yerr=inst_stds, fmt='--', label='Unit-wise constraints')
    ...
    if saveImg:
        ...
        filename = os.path.join(subdirectory, f'corrpref_{type}_{dist}_beta{beta}_iter{iter}.png')
