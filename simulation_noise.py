...
def util_noise_simulation(n, p, k_inst, gamma, phi, test, dist, beta, type, iter):
    ...
    sns.heatmap(heatmap_data, annot=True, fmt=".3f", xticklabels=sigma_a, yticklabels=sigma_b, 
                cmap='coolwarm', vmin=0.0, vmax=1.0, cbar = False)
    plt.xlabel("$\sigma_a$")
    plt.ylabel("$\sigma_b$")
    plt.tight_layout()
    ...
    filename = os.path.join(subdirectory, f'heatmap_{test}_{type}_{dist}.png')
...
def beta_noise_simulation(n, p, k_inst, gamma, phi, dist, type, iter, saveImg=False, showImg=True, std=0.1):
    ...
    for beta in betas:
        ...
        for i in range(iter):
            ...
            control_beta_dist = [beta] * len(utils_beta)
            ...
            if type == 'ptop1':
                ...
                utils_beta = utils_beta / beta_dist
                utils_total[cut:] = utils_beta[::]
                gs_res.append(u(gs_temp, utils_total[::], cut = cut, beta = 1))
                group_res.append(u(group_temp, utils_total[::], cut = cut, beta = 1))
                inst_res.append(u(inst_temp, utils_total[::], cut = cut, beta = 1))

                control_gs_res.append(u(control_gs_temp, control_utils_total[::], cut = cut, beta = 1))
                control_group_res.append(u(control_group_temp, control_utils_total[::], cut = cut, beta = 1))
                control_inst_res.append(u(control_inst_temp, control_utils_total[::], cut = cut, beta = 1))
            else:
    ...
    plt.errorbar(betas, gs_means, yerr=gs_stds, fmt='-', label='Unconstrained')
    plt.errorbar(betas, group_means, yerr=group_stds, fmt='-', label='Group-wise constraints')
    plt.errorbar(betas, inst_means, yerr=inst_stds, fmt='-', label='Unit-wise constraints')

    plt.errorbar(betas, control_gs_means, yerr=control_gs_stds, fmt='--', label='Unconstrained control')
    plt.errorbar(betas, control_group_means, yerr=control_group_stds, fmt='--', label='Group-wise control')
    plt.errorbar(betas, control_inst_means, yerr=control_inst_stds, fmt='--', label='Unit-wise control')

    # Add labels and title
    plt.xlabel(r'$\beta$')
    if type == 'u':
        plt.ylabel("Fraction of optimal latent utility")
    else:
    ...
    if saveImg:
        ...
        filename = os.path.join(subdirectory, f'{type}_{dist}_phi{phi}_iter{iter}_std{std}.png')
        plt.savefig(filename)
