...
def simulation_relaxedbounds(n, p, k_inst, gamma, phi, dist, beta, type, iter, saveImg=False, showImg=True):
    ...
    # 3 experiment bars
    plt.errorbar(relaxed_bound, gs_means, yerr=gs_sems, fmt='--', label='Unconstrained (control)')
    plt.errorbar(relaxed_bound, group_control_means, yerr=group_control_sems, fmt='--', label="Group-wise (control)")
    plt.errorbar(relaxed_bound, inst_control_means, yerr=inst_control_sems, fmt='--', label="Unit-wise (control)")
    plt.errorbar(relaxed_bound, group_means, yerr=group_sems, fmt='-', label='Group-wise constraints')
    plt.errorbar(relaxed_bound, inst_means, yerr=inst_sems, fmt='-', label='Unit-wise constraints')

    # Control is at the right end of x-axis, where that is perfect (also unconstrained)


    # Axes
    plt.xlabel(r"Lower bound ($\alpha$)")
    if type == 'u':
    ...
    if saveImg:
        ...
        filename = os.path.join(subdirectory, f'{dist}_{type}_{beta}_relaxedbounds.png')
